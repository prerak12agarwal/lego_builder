...
type Config = { configured: boolean; converterConfigured?: boolean; latest: JobView | null; latestReady: JobView | null };
...
export default function Workspace() {
  ...
  useEffect(() => {
    ...
    void api<Config>("/api/config").then(async value => {
      ...
      const newProject = params.get("new") === "1";
      const latestIsActive = Boolean(value.latest && ACTIVE_STATES.includes(value.latest.state));
      let existing = newProject && !latestIsActive ? null : value.latest;
      if (newProject && latestIsActive) setError("Your current generation is still in progress. Reopen it here before starting another project.");
  ...
  function selectJob(value: JobView) {
    ...
    history.replaceState(null, "", `/build?job=${encodeURIComponent(value.id)}${sameJob && conversion ? `&conversion=${encodeURIComponent(conversion)}` : ""}`);
  }
  ...
  async function generate() {
    ...
    history.replaceState(null, "", `/build?request=${key}`);
    try { selectJob(await api<JobView>("/api/jobs", { method: "POST", headers: { "Content-Type": "image/png", "Idempotency-Key": key }, body: input })); }
      ...
      try {
        ...
        else if ((e as { status?: number }).status) { setPendingRequest(null); history.replaceState(null, "", job ? `/build?job=${encodeURIComponent(job.id)}` : "/build"); }
  ...
  async function remove() {
    if (!job) return;
    setBusy(true); setError("");
    try { await api(`/api/jobs/${job.id}`, { method: "DELETE" }); setJob(null); setPendingRequest(null); history.replaceState(null, "", "/build"); }
    catch (e) { setError(message(e)); } finally { setBusy(false); }
  }
  ...
  return <div className="reconstruction-workspace">
    <main className="workbench">
      <div className="page-heading"><div><p className="eyebrow">PHOTO TO BUILD</p><h1>Build from your photo.</h1><p className="muted">Reconstruct its shape and colors, then create a LEGO model.</p></div><span className="step-pill">INTEGRATION PREVIEW</span></div>
      <ol className="pipeline-stages" aria-label="Build pipeline"><li className={job || photo ? "stage-done" : "stage-current"}><span>1</span><div><strong>Reference photo</strong><small>{job || photo ? "Photo selected" : "Choose an image"}</small></div></li><li className={ready ? "stage-done" : active ? "stage-current" : ""}><span>2</span><div><strong>3D reconstruction</strong><small>{ready ? "Mesh saved" : active ? "TRELLIS processing" : "Generate with TRELLIS"}</small></div></li><li className={hasLdrResult ? "stage-done" : ready ? "stage-current" : ""}><span>3</span><div><strong>LEGO conversion</strong><small>{hasLdrResult ? "Brick model saved" : ready ? config?.converterConfigured ? "Ready to convert saved mesh" : "Converter not configured" : "Waiting for 3D model"}</small></div></li><li className={hasLdrResult ? "stage-done" : ""}><span>4</span><div><strong>Model & steps</strong><small>{hasLdrResult ? "Parts and draft steps available" : "Available after conversion"}</small></div></li></ol>
      <div className="work-grid">
        <aside className="input-panel">
          ...
          <p className="setup-note">{signedOut ? "Sign in to keep your models private and recover saved jobs." : !config && error ? error : !config ? "Connecting to your workspace…" : !config.configured ? "Generation needs to be configured by the site owner." : "Powered by TRELLIS."}</p>
        ...
        <section className="model-panel" aria-label="3D model workspace">
          ...
          {ready && job ? <PipelineWorkspace key={job.id} job={job} converterConfigured={config?.converterConfigured} onResultAvailable={setHasLdrResult}/> : <div className="empty-stage"><Empty><EmptyHeader><EmptyMedia><span className="stage-icon">{active && job?.state !== "unknown" ? <LoaderCircle size={38} className="spinner"/> : <Box size={38} strokeWidth={1.25}/>}</span></EmptyMedia><EmptyTitle>{stage?.[0] ?? "Your model starts with a photo"}</EmptyTitle><EmptyDescription>{stage?.[1] ?? "Choose one clear photo, then generate its 3D shape. Your mesh and converter handoff will be saved here."}</EmptyDescription></EmptyHeader></Empty></div>}
          {job && <div className="job-details" aria-live="polite">
            {ready ? <><div className="model-summary"><strong>{job.triangles?.toLocaleString()} triangles</strong><span>Physical scale unknown · LEGO compatibility unchecked</span>{job.bounds && <span>Bounds: {job.bounds.max.map((v, i) => (v - job.bounds!.min[i]).toPrecision(3)).join(" × ")} model units</span>}</div><div className="download-row"><Button asChild variant="default"><a href={`/api/jobs/${job.id}/files/glb`} download><Download size={16}/>{job.appearance === "preserved" ? "Download color GLB" : "Download GLB"}</a></Button><Button asChild variant="outline"><a href={`/api/jobs/${job.id}/files/${job.appearance === "preserved" ? "obj-bundle" : "obj"}`} download><Download size={16}/>{job.appearance === "preserved" ? "Download textured OBJ" : "Download geometry OBJ"}</a></Button><Button asChild variant="outline"><a href={`/api/jobs/${job.id}/files/manifest`} download><Download size={16}/>Model details</a></Button></div></> : <p>{job.message ?? (active ? "Keep this page open to collect the result, or return through this job link promptly." : "Choose another photo to make a new model.")}</p>}
            <div className="job-actions"><a href={`/build?job=${encodeURIComponent(job.id)}`}>Saved job link</a><AlertDialog><AlertDialogTrigger asChild><Button variant="ghost" disabled={busy}><Trash2 size={15}/>Remove job</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Remove this job and its files?</AlertDialogTitle><AlertDialogDescription>This removes your saved photo and downloads from this workspace. An active provider request may still finish and be charged. Provider copies follow fal’s retention settings. If submission is uncertain, check your fal history first.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Keep job</AlertDialogCancel><AlertDialogAction onClick={() => void remove()}>Remove job</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>
          </div>}
          <div className="viewer-footer"><span><RotateCcw size={16}/> Drag to rotate · Scroll to zoom</span><span>Inspection workspace</span></div>
        </section>
      </div>
      {previousReady && previousReady !== job?.id && <p className="previous-model"><a href={`/build?job=${encodeURIComponent(previousReady)}`}>Open your previous completed model</a></p>}
      <footer className="page-footer"><span>Photos become approximate shapes. Hidden surfaces are inferred.</span><span>LEGO assembly remains unverified.</span></footer>
    </main>
  </div>;
}
