...
export default function Workspace() {
  ...
  return <div className="reconstruction-workspace">
    <main className="workbench">
      ...
      <ol className="pipeline-stages" aria-label="Build pipeline"><li className={job || photo ? "stage-done" : "stage-current"}><span>1</span><div><strong>Reference photo</strong><small>{job || photo ? "Photo selected" : "Choose an image"}</small></div></li><li className={ready ? "stage-done" : active ? "stage-current" : ""}><span>2</span><div><strong>3D reconstruction</strong><small>{ready ? "Mesh saved" : active ? "TRELLIS processing" : "Generate with TRELLIS"}</small></div></li><li><span>3</span><div><strong>LEGO conversion</strong><small>{hasLdrResult ? "LDraw result saved" : config?.converterConfigured ? "Ready to convert saved mesh" : "Converter not configured"}</small></div></li><li className={hasLdrResult ? "stage-current" : ""}><span>4</span><div><strong>Model & steps</strong><small>{hasLdrResult ? "LDraw received for inspection" : "Ready to receive LDraw"}</small></div></li></ol>
