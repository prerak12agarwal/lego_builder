...
// Identifies only the current caller for the pilot owner's Site-specific setup.
// Never accesses model storage, provider credentials or allowlists.
export async function GET(request: Request) {
  const user = await getChatGPTUser();
  if (request.headers.get("accept")?.includes("text/html")) {
    const escape = (value: string) => value.replace(/[&<>"']/g, character => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character]!));
    const content = user
      ? `<h1>Your workshop account</h1><p>${escape(user.displayName)}</p><p>Account ID for pilot access: <code>${escape(user.userId)}</code></p><p><a href="/build">Return to your workshop</a></p>`
