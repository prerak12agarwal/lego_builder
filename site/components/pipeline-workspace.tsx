...
export function PipelineWorkspace({ job, onResultAvailable, converterConfigured = false }: { job: JobView; onResultAvailable: (value: boolean) => void; converterConfigured?: boolean }) {
    ...
    void (async () => {
      try {
        ...
        if (!control.signal.aborted) {
          ...
          history.replaceState(null, "", `/build?job=${encodeURIComponent(job.id)}&conversion=${encodeURIComponent(value.id)}`);
  ...
  async function runConversion() {
    ...
    try {
      ...
      if (selected.settings.targetParts !== desired.targetParts || selected.settings.inputUpAxis !== desired.inputUpAxis || selected.settings.targetSizeStuds !== undefined) {
        ...
        history.replaceState(null, "", `/build?job=${encodeURIComponent(job.id)}&conversion=${encodeURIComponent(selected.id)}`);
      }
  ...
  return <div className="pipeline-workspace">
    ...
    <details className="converter-tools"><summary>Converter handoff <span>{loading ? "Preparing…" : hasResult ? "Result saved" : "Integration tools"}</span></summary><div className="converter-tools-body">
      <p>Download the mesh and request details, or attach an LDraw result produced from this mesh. Manually supplied files remain unverified.</p>
      ...
      <p className="handoff-snapshot"><a href={`/build?job=${encodeURIComponent(job.id)}&conversion=${encodeURIComponent(conversion.id)}`}>Link to this saved handoff</a></p>
      {previous && <Button variant="ghost" onClick={() => { setConversion(previous); setSettings(previous.settings); selectedRequest.current = previous.id; history.replaceState(null, "", `/build?job=${encodeURIComponent(job.id)}&conversion=${encodeURIComponent(previous.id)}`); setTab("model"); }}>View previous saved result</Button>}
      </>}
    </div></details>
  </div>;
}
