...
export function settingsJson(settings: ConversionSettings) { return JSON.stringify(settings.targetParts !== undefined ? { inputUpAxis: settings.inputUpAxis, targetParts: settings.targetParts } : { inputUpAxis: settings.inputUpAxis, targetSizeStuds: settings.targetSizeStuds }); }
...
export class Conversions {
  ...
  async run(owner: string, sourceJobId: string, id: string) {
    ...
    const pinned = sourceHash(await this.parent(owner, sourceJobId));
    ...
    const result = await runConverter(this.env, { schemaVersion: 1, obj, sourceObjSha256: row.source_obj_sha256, settingsSha256: row.settings_hash, settings: { targetParts: settings.targetParts, inputUpAxis: settings.inputUpAxis } });
