...
## Automatic piece-target requests

New requests use `settings: {"targetParts":2000,"inputUpAxis":"y"}` (100–2,200 pieces and explicit x/y/z). The canonical hash serializes keys in the order `inputUpAxis`, `targetParts`. Existing stud-based requests retain their original hash and manual semantics.

`POST /api/jobs/:sourceJobId/conversion/:requestId/run` requires the signed-in owner and same Origin. It reads the pinned OBJ from private R2, verifies its hash, calls the configured HTTPS Python converter with server-only credentials, checks result hash echoes and invokes the existing immutable result publisher. A legacy stud request must be replaced by an explicit piece-target request before automatic execution. No photo-generation request is repeated by this action.
