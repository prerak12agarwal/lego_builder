  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        if (stopped) { gltf.scene.traverse(child => { if (child instanceof THREE.Mesh) child.geometry.dispose(); }); return; }
        ...
        const material = new THREE.MeshStandardMaterial({ color: "#bacbe3", roughness: 0.78, metalness: 0.04, side: THREE.DoubleSide });
        gltf.scene.traverse(child => {
          if (child instanceof THREE.Mesh) {
            child.geometry.computeVertexNormals();
            const old = Array.isArray(child.material) ? child.material : [child.material];
            old.forEach(m => m.dispose()); child.material = material;
          }
        });
        ...
        dispose = () => {
          observer.disconnect(); controls.dispose(); material.dispose();
          gltf.scene.traverse(child => { if (child instanceof THREE.Mesh) child.geometry.dispose(); });
      ...
      } catch { if (!stopped) setError("The 3D preview could not open on this device. Your model is saved; you can still download it below."); }
    }
    void start();
