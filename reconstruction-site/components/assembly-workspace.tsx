...
export function AssemblyWorkspace({ result, tab }: { result: SavedAssembly; tab: AssemblyTab }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const revision = await importLDraw("model.ldr", new TextDecoder("utf-8", { fatal: true }).decode(bytes), await colors.text(), async (name, signal) => {
          const part = await fetch(`/api/ldraw-part?file=${encodeURIComponent(name)}`, { signal });
          const data = await part.json() as { path: string; text: string; error?: string };
          if (!part.ok) throw Error(`${name}: ${data.error ?? "Part geometry could not be loaded."}`);
          return data;
        }, control.signal, setProgress);
  ...
  return <div className="assembly-content">
    ...
    <p className="assembly-attribution">Geometry: <a href="https://library.ldraw.org/" target="_blank" rel="noreferrer">LDraw contributors</a> · <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noreferrer">CC BY 4.0</a></p>
