...
const defaultSettings: ConversionSettings = { targetSizeStuds: 24, inputUpAxis: "unspecified" };
const errorMessage = (error: unknown) => error instanceof Error ? error.message : "The request could not be completed.";

export function PipelineWorkspace({ job, onResultAvailable }: { job: JobView; onResultAvailable: (value: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!value) value = await request<ConversionView>(base, { method: "POST", signal: control.signal, headers: { "Content-Type": "application/json", "Idempotency-Key": `handoff-${job.id}` }, body: JSON.stringify({ schemaVersion: 1, settings: defaultSettings }) });
  ...
  async function createAttempt() {
    const attempt = pending ?? { key: crypto.randomUUID(), settings: { ...settings } };
  ...
  return <div className="pipeline-workspace">
    <div className="pipeline-status" role="status"><span className="pipeline-status-icon">{hasResult ? <Layers size={20}/> : <Check size={20}/>}</span><div><strong>{hasResult ? "LDraw result received" : "3D mesh ready · Awaiting converter"}</strong><p>{hasResult ? "Inspect the supplied model, parts and authored steps. Buildability remains unverified." : "Your mesh is saved. LEGO conversion will use this result when the converter is connected."}</p></div></div>
    ...
    <details className="converter-tools"><summary>Converter handoff <span>{loading ? "Preparing…" : hasResult ? "Result saved" : "Integration tools"}</span></summary><div className="converter-tools-body">
      ...
      {loading ? <p role="status"><LoaderCircle className="spinner" size={16}/> Preparing your saved handoff…</p> : conversion && <><div className="handoff-settings"><div><label htmlFor="target-size">Target longest dimension (studs)</label><Input id="target-size" type="number" min={4} max={48} step={1} value={settings.targetSizeStuds} disabled={busy || Boolean(pending)} onChange={event => setSettings(value => ({ ...value, targetSizeStuds: Number(event.target.value) }))}/></div><div><label htmlFor="up-axis">Source upright axis</label><Select value={settings.inputUpAxis} disabled={busy || Boolean(pending)} onValueChange={value => setSettings(settings => ({ ...settings, inputUpAxis: value as InputUpAxis }))}><SelectTrigger id="up-axis"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="unspecified">Not confirmed</SelectItem><SelectItem value="x">X axis</SelectItem><SelectItem value="y">Y axis</SelectItem><SelectItem value="z">Z axis</SelectItem></SelectContent></Select></div><Button variant="outline" disabled={busy || !Number.isInteger(settings.targetSizeStuds) || settings.targetSizeStuds < 4 || settings.targetSizeStuds > 48} onClick={() => void createAttempt()}>{pending ? "Retry saving handoff" : "Save new handoff"}</Button></div>
      <p className="handoff-snapshot">Saved request: {conversion.settings.targetSizeStuds} studs · Up axis {conversion.settings.inputUpAxis === "unspecified" ? "not confirmed" : conversion.settings.inputUpAxis.toUpperCase()}. Changing settings takes effect after saving a new handoff. Supported sizes and build quality depend on the converter.</p>
      ...
      <p className="handoff-snapshot">Each result is saved to its request and cannot be overwritten. Start a new handoff to supply another result. No Python execution is connected yet.</p>
