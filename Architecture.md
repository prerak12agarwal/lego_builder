...
Hosting on ChatGPT Sites is a confirmed founder preference. The reconstruction pilot has application and geometry-processing source in [reconstruction-site](reconstruction-site/package.json) and one successful [local live bottle conversion](reconstruction-site/test/live-evidence.md). Hosted publishing and validation remain pending; the mesh-to-LEGO converter remains planned. Dependency manifests and lockfiles own exact versions. Keep local test evidence distinct from hosted release claims and design intent.
