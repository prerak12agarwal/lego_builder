...
## Current exterior vehicle generation decision

Founder-approved direction: use the supplied Cybertruck OBJ to create a detailed exterior interpretation at approximately 2,000 real pieces, with a working target range of 1,800–2,200. Preserve the source file, recover its exterior without requiring a watertight input, and synthesize useful internal structure. Interior seats and dashboard fidelity are unnecessary. No external stand or rescue support is added. This expands the rectangular prototype through a separate generation path; it does not remove the existing path or silently broaden its validation claims.
...
The founder has authorized export of a digital candidate with explicit unverified assembly claims. This does not relax hard invalid-part, malformed-transform or artifact-consistency checks, and does not make a candidate buildability-verified. Keep the old grid schema, reader, converter and tests intact. Dispatch by schema instead of reinterpreting existing integer grid placements as LDraw coordinates. No database migration is involved; the new schema/catalog/algorithm identities create new revisions and old outputs retain their original meaning. Independent QA checks the new artifacts and claims before handoff, with physical assembly evidence still required for buildability claims.
