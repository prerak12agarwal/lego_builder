...
The independent UI source and its own Sites manifest remain under [site/](site/). Its publication uses a separate source snapshot rooted at that directory, with a per-command credential; it does not replace GitHub origin or publish the repository’s internal reference assets. Recreate that snapshot from the reviewed UI source for each publication. The UI preview and reconstruction pilot are separate applications; merging their source does not connect their user flows.
...
### Confirmed integration direction; adapter design proposed

The founders have requested a connected single-image → TRELLIS mesh → Prerak's Python converter → `.ldr` → model and authored-step workspace. This approves joining the workstreams; it does not establish converter compatibility, approve a new paid execution provider, or relax validation and physical-build gates. The existing mesh-only pilot and local LDraw inspector retain their documented behavior until the integration is implemented.

Reusable boundaries already exist: [reconstruction jobs](reconstruction-site/lib/jobs.ts) save checked geometry-only OBJ and its checksum manifest; [LDraw import](site/lib/ldraw/import.ts) and [render preparation](site/lib/ldraw/render.ts) resolve and display actual parts with root-authored steps. They are separate applications: [the consumer page](site/app/page.tsx) still reads its sample assembly, and [the test bench](site/components/admin/test-bench.tsx) only accepts a local file. No existing API dispatches Python conversion or publishes a saved LEGO revision to that workspace.

The separately reviewed [converter source at `69bd61e`](https://github.com/prerak12agarwal/lego_builder/tree/69bd61e) supplies a Python CLI accepting OBJ/STL, target size and up-axis. Successful output includes `model.ldr`, canonical `model.json`, `sequence.json`, bill of materials and `validation.json`; handled conversion failures emit a validation report with a nonzero exit. Its LDR has a revision header and a STEP boundary after each placement. Its current geometry gate requires watertight, consistently wound, nondegenerate input with nonzero enclosed volume, which the reconstruction exporter does not guarantee. The restricted catalog uses red parts and has no complexity-preset interface. These are source findings, not evidence that this revision converts real TRELLIS output successfully; inspect the merged executable contracts before writing its adapter.

Proposed minimum adapter contracts:

| Boundary | Required meaning before implementation |
| --- | --- |
| Mesh to converter | One immutable OBJ plus its manifest, identified by reconstruction job and checksum. Resolve target size in studs, upright-axis transform, permitted catalog/palette and converter version into a persisted settings snapshot. Current exports have provider-native axes, unknown physical scale and no MTL/texture or color transfer; compatibility must be tested against those bytes. |
| Python execution | Wrap the agreed executable entry point in an authenticated asynchronous worker with pinned dependencies, bounded runtime/memory/concurrency, isolated temporary files, progress/error reporting and cancellation behavior. Sites calls this service through an application adapter; selecting its host and budget remains open. A local runner can establish file compatibility before hosted execution is selected. |
| Durable conversion job | Create a separate owner-scoped conversion job linked to the ready reconstruction artifact. Persist the worker task ID, immutable settings, attempts, state, lease and error category. Derive deduplication from input checksum and resolved settings/version; retry conversion or result collection without rerunning TRELLIS. Mesh-ready and LEGO-ready are distinct outcomes. Reuse the pilot's request-driven reconciliation pattern only with its existing collection limitation disclosed. |
| Worker artifact access | Transfer bounded files server-to-server, or use narrowly scoped expiring job credentials for private artifact access. An external worker must not depend on an interactive ChatGPT browser session, receive broad storage credentials, or accept arbitrary browser-supplied download URLs. Bind returned output to the same job and expected input checksum. |
| Converter result | Require LDR plus canonical placements, instruction membership, inventory, validation report and provenance/checksums. LDR alone can support inspection but cannot establish the validated LEGO revision contract. Store outputs privately and publish the revision only after schema, catalog, geometry, inventory and step-consistency gates pass; retain rejected candidates separately for diagnostics. |
| Workspace adapter | Fetch the saved owner-authorized result and reuse LDraw parsing/rendering in the main model window. Map imported placement identities to the canonical revision and verify transforms, parts/colors and step membership against it. The current importer creates a random session revision ID; it must not become the persisted model identity. Full model, parts and instructions use the same selected immutable result, with the prior result preserved on failure. |

For the first compatible result, use flat root part placements and explicit `0 STEP` boundaries in LDR. The existing importer supports root `ROTSTEP` boundaries but does not apply their camera rotations; submodel steps are flattened into the parent's step and nested instruction callouts are not displayed. Every placement must appear exactly once in the ordered steps, and the final cumulative step must equal the full model. Without authored boundaries the existing missing-step warning applies; the viewer does not invent a sequence. Confirm converter output fits the executable [import limits](site/lib/ldraw/model.ts), currently 5 MiB and 2,500 placements plus bounded dependency complexity, before selecting target sizes. Part-library resolution is an additional availability dependency.

Proposed deployment approach: integrate the consumer shell, reconstruction routes/storage and reusable LDraw viewer into one Sites application and one ownership boundary. Choose the surviving deployment and migrate bindings/artifacts deliberately; merging source directories does not merge the two Sites databases or private stores. Add conversion tables through new migrations, preserve reconstruction rows and tombstones, and keep existing mesh downloads usable. Roll out behind explicit configuration after owner-isolation, deletion/cancellation races and reload recovery pass. Deletion must cover downstream jobs and artifacts as well as the source job.

Verify the seam first with an actual prepared OBJ from reconstruction, the pinned Python converter, and the resulting LDR in the current importer. Record input/output hashes, axis/size interpretation, part counts, ordered placement coverage and failure reports. Then exercise the authenticated hosted upload-to-steps flow, interrupted/repeated requests, converter failure without repeat inference, missing dependencies/steps and maximum supported output. Measure each stage and total latency/cost. This establishes integration evidence; Studio compatibility and physical-build evidence remain separate release requirements.
