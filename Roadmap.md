...
Approved MVP scope for new jobs on the canonical Workshop is a narrow single-image upload → real generation → interactive appearance-preserving `GLB` preview and owner download → reusable geometry-only `OBJ` and manifest handoff. Preserve supported original materials and textures from TRELLIS when present and valid. A mesh with no authored appearance receives a truthful neutral fallback. Authored appearance that is malformed, unsupported, or cannot be decoded produces a clear collection or preview failure rather than a neutral successful result; available artifacts remain retained, and collection or preview recovery never triggers another paid generation. A successful result must be the user's returned mesh, with inferred hidden surfaces, uncertain appearance and unknown real-world scale disclosed. This does not claim exact color recovery or add color/texture fitting to the LEGO converter. Actual converter compatibility requires validation. Failures, uncertain geometry, and provider limitations remain visible. Minimum private per-user jobs and artifact storage support secure processing and recovery, while full account management and project-library UX remain outside this slice.
...
Confirmed founder direction: make [the original workshop Site](https://lego-builder-workshop.dragonjjk.chatgpt.site/) the canonical destination for the existing single-image upload, fal-hosted original TRELLIS, appearance-preserving ordinary `GLB`, retained geometry-only `OBJ`, the existing generic Python converter, private project recovery and LDraw model/parts/instructions inspection. New canonical jobs preserve supported original materials/textures in their ordinary-model viewer and owner download without claiming exact source color or reliable hidden-surface appearance. The LEGO converter contract remains geometry-only. Preserve the workshop's established styling, sample workbench and local Admin LDraw inspection. Product F8 owns the acceptance criteria. Preserve the implemented manual handoff while adding an authenticated, bounded HTTPS Python execution boundary; new generic requests target about 2,000 parts, retain explicit orientation and remain distinct from legacy size-based handoffs. Multiple-image reconstruction remains deferred follow-on work.
