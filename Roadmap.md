...
Confirmed founder direction: use the dedicated integration branch to connect the user-visible pipeline around the converter while Prerak optimizes that converter separately. This slice joins image upload, fal-hosted TRELLIS, the retained geometry-only `OBJ`, a stable deferred-converter handoff, and the LDraw model/parts/steps result surfaces. Product F8 owns the acceptance criteria. Integrating or optimizing the converter from PR #5 is outside this slice.
...
Automatic Python execution remains unimplemented, and its host and deployment boundary remain open. The current size input is a 4–48-stud request bound rather than a validated quality range, and source orientation defaults to unknown. A narrow local bottle check reached the persisted pending state without another provider request, and an isolated fixture exercised the LDraw result UI; independent [pipeline QA](reconstruction-site/test/pipeline-evidence.md) passes for the surrounding slice. Hosted acceptance and real converter execution remain open.
...
| Q8 | Integrated job timing, retention, downstream retry limits and Python execution host | Preserve each successful stage so an eligible failure can resume without another paid reconstruction; set exact limits and the execution boundary after converter and hosted benchmarks | External pipeline pilot |
