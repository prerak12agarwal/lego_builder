...
Proposed implementation boundary: compact, static display objects, a restricted catalog of ordinary bricks/plates, a limited manufactured-color palette, and a small model size. Explain that hidden surfaces and their colors are inferred. Favor recognizable, buildable approximations and guided choices over expert modeling controls. Exact supported categories, photo count and fidelity targets remain open.
...
| --- | --- | --- |
| P0 — Mesh-to-brick MVP | Retain the ten-shape release benchmark and established rectangular baseline. Exercise general exterior recovery and broader supported part fitting across distinct subjects. Implement normalization, discretization, color sampling from a paired appearance source, manufactured-palette quantization, fitting, applicable structural and sequence validation, LDraw export and exact inventory; distinguish digital candidates from validated assemblies. Test several target sizes and compare with BrickLink Studio Sculpture where practical. | Every released candidate imports into BrickLink Studio and has matching colored placements and part/color inventory, valid catalog IDs/colors/orientations, no collisions or floating components, and a feasible MVP placement order. Record shape and color resemblance, parts, unique lots, runtime, validator failures and manual repairs for every candidate; physically build at least two difficult outputs. |
...
| P2 — Image conversion | Upload/preprocess images, reconstruct approximate geometry and appearance, retain a paired validated GLB plus exact geometry handoff, then run the proven color-aware fitter and validators asynchronously. | Real images produce traceable colored model revisions on the agreed benchmark; geometry, appearance and palette limitations are visible; measured cost and timing satisfy established targets. |
| P3 — Complete user journey | Connect project flow, configuration, progress/recovery, colored model inspection, part/color export, draft instructions, persistence and access control. | A user completes upload → generate → inspect → export → follow color-consistent instructions; the same immutable colored revision survives retries and changes. |
...
Approved MVP scope for new jobs on the canonical Workshop is a narrow single-image upload → real generation → interactive appearance-preserving `GLB` preview and owner download → portable `OBJ` / `MTL` / texture ZIP plus reusable geometry-only `OBJ` and manifest → paired GLB-and-OBJ LEGO handoff. Preserve supported original materials and textures from TRELLIS when present and valid. A mesh with no authored appearance receives a truthful neutral fallback. Authored appearance that is malformed, unsupported, or cannot be decoded produces a clear collection or preview failure rather than a neutral successful result; available artifacts remain retained, and collection or preview recovery never triggers another paid generation. The color-aware converter maps the reconstructed visible appearance to the closest supported manufactured LEGO colors and carries them through the canonical LDraw revision, inventory and draft instructions. A successful result must be the user's returned mesh, with inferred hidden surfaces, approximate palette mapping and unknown real-world scale disclosed. Actual converter compatibility requires validation. Failures, uncertain geometry, and provider limitations remain visible. Minimum private per-user jobs and artifact storage support secure processing and recovery, while full account management and project-library UX remain outside this slice.
...
Current implementation status before this approved extension: source exists for normalized single-image upload, private per-user jobs, centralized pilot quotas, duplicate-submit protection, request-driven reopen recovery, saved source reuse, prior-ready navigation, interactive appearance-preserving preview, and private `GLB`/geometry-only `OBJ`/manifest downloads with bounds and unknown-scale disclosure. Appearance preservation for new canonical Workshop jobs is implemented with bounded validation and color GLB downloads; [color preservation QA](docs/qa-trellis-color.md) owns that verification scope. Portable OBJ packaging and color-aware LEGO fitting require implementation and new evidence. Existing neutral or appearance-stripped saved jobs remain readable as-is; no automatic paid reconstruction will recreate missing color. App files are retained until user removal. Provider requests ask for private access, no fal input/output storage, and 24-hour output expiry; unfinished cancellation may still be charged.
...
Confirmed founder direction: make [the original workshop Site](https://lego-builder-workshop.dragonjjk.chatgpt.site/) the canonical destination for the existing single-image upload, fal-hosted original TRELLIS, appearance-preserving ordinary `GLB`, portable colored OBJ package, retained exact geometry-only `OBJ`, the generic Python converter, private project recovery and color-consistent LDraw model/parts/instructions inspection. New canonical jobs preserve supported original materials/textures in their ordinary-model viewer and owner downloads without claiming exact source color or reliable hidden-surface appearance. The LEGO converter receives the paired validated GLB and geometry-only OBJ, maps visible appearance to a bounded manufactured LEGO palette, and publishes one immutable colored revision across Model, Parts and Draft instructions. Preserve the workshop's established styling, sample workbench and local Admin LDraw inspection. Product F8 owns the acceptance criteria. Preserve the implemented manual handoff while extending the authenticated, bounded HTTPS Python execution boundary; new generic requests target about 2,000 parts, retain explicit orientation and remain distinct from legacy size-based handoffs. Multiple-image reconstruction remains deferred follow-on work.
...
The approved source-color pipeline is implemented with [independent verification](docs/qa-end-to-end-colors.md). Its fixture results establish bounded color transfer and artifact consistency; they do not waive hosted, Studio, physical-build or general-quality gates.
