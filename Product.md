...
The public shell, examples, hand-authored sample workbench and browser-local Admin LDraw inspection remain available without sign-in. Every user signed in with ChatGPT may start provider-backed generation and use the full workflow. Accessing, resuming or removing a private job and its artifacts requires the same Sites identity that owns that job. The sign-in transition returns the user to the intended new-project or saved-project context. Existing per-user and site-wide quotas remain unchanged; expanding signed-in access does not expand those limits. Provider keys and converter credentials never reach the browser.
