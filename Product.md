...
4. The successful mesh is preserved as reusable artifacts with its source association and generation status. The user can download the appearance-bearing `GLB` shown in the viewer and the geometry-only `OBJ` plus manifest targeted by the F0 input contract. The later LEGO conversion step consumes that same `OBJ` rather than reconstructing the image again, once compatibility has been validated.
...
- Preserve the source association, source-photo hash, and the relationship between the downloadable `GLB`, geometry-only `OBJ`, and manifest. The `OBJ` converter handoff remains geometry-only; this decision does not add color or texture fitting to the LEGO model. The interface does not claim exact source-photo color recovery, physically accurate materials, or accurate unseen surfaces. Inferred hidden-surface appearance is subject to the same approximation warning as inferred geometry.
...
MVP non-goals are LEGO brick fitting, parts lists, assembly instructions, new account-management UI, a project-library experience, manual mesh editing, application-authored texture generation or repair, exact color-fidelity claims, colored LEGO-part fitting, arbitrary-object guarantees, provider-specific advanced controls, live pricing, and purchasing. Existing Sites identity plus minimum private job and artifact storage are in scope for secure generation and recovery. Hosted runtime validation, production retention verification, generation timeout, and acceptable generalized quality, latency and cost thresholds remain pending.
...
Appearance preservation applies to the ordinary reconstructed 3D output only. Colored or textured LEGO fitting remains outside this slice, and the converter continues to receive the geometry-only `OBJ` contract.
