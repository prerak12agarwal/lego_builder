...
The public shell, examples, hand-authored sample workbench and browser-local Admin LDraw inspection remain available without sign-in. Every user signed in with ChatGPT may start provider-backed generation and use the full workflow. Accessing, resuming or removing a private job and its artifacts requires the same Sites identity that owns that job. The sign-in transition returns the user to the intended new-project or saved-project context. The canonical Workshop has no per-person rolling count quota on generation attempts. It retains a maximum of 20 new generation attempts across the site in a rolling 24-hour window, one unresolved generation per user, and idempotent duplicate-submit protection. The separate image-to-3D pilot retains its existing quota behavior. Provider keys and converter credentials never reach the browser.
