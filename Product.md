...
The public shell, examples, hand-authored sample workbench and browser-local Admin LDraw inspection remain available without sign-in. Every user signed in with ChatGPT may start provider-backed generation and use the full workflow. Accessing, resuming or removing a private job and its artifacts requires the same Sites identity that owns that job. The sign-in transition returns the user to the intended new-project or saved-project context. The canonical Workshop has no per-person rolling count quota on generation attempts. It retains a maximum of 100 new generation attempts across the site in a rolling 24-hour window, one unresolved generation per user, and idempotent duplicate-submit protection. The separate image-to-3D pilot retains its existing quota behavior. Provider keys and converter credentials never reach the browser.
...
3. Ready opens an interactive mesh preview with orbit, zoom, pan, fit-to-view, and camera reset. For new canonical Workshop jobs, it displays supported materials and textures from the returned TRELLIS `GLB` when they are present and valid. A mesh with no authored appearance uses a labeled neutral fallback. Authored appearance that is malformed or unsupported produces a clear collection or preview failure instead of a gray successful result. The workspace identifies a displayed result as an approximate reconstructed mesh and exposes obvious warnings such as missing or uncertain hidden surfaces.
4. The successful mesh is preserved as reusable artifacts with its source association and generation status. The user can download the appearance-bearing `GLB` shown in the viewer, a portable `OBJ` / `MTL` / texture package for color-capable tools, and the existing geometry-only `OBJ` plus manifest. LEGO conversion consumes the validated `GLB` for appearance together with the exact geometry-only `OBJ`; it does not reconstruct the image again.
...
- For new jobs on the canonical Workshop, the ready state renders the returned reconstructed mesh with supported original materials and textures when the stored TRELLIS `GLB` supplies valid authored appearance. It provides the result format, approximate bounds when available, an appearance-bearing `GLB` download, a portable `OBJ` / `MTL` / texture ZIP, and the reusable geometry-only `OBJ` plus manifest. The package uses relative references and contains all files needed to reproduce the supported appearance without fetching external resources. A mesh with no authored appearance remains usable with a neutral fallback and says that appearance is absent rather than implying color was recovered. If authored materials or textures are present but malformed, unsupported, or fail to decode, collection or preview fails clearly; the application does not silently remove that appearance and publish a neutral success.
- When authored base color is present, the ordinary-model workspace provides a color-inspection presentation that does not make the result appear black because metallic defaults or missing environment reflections dominate the render. A separately labeled lit/material presentation may remain available, but it must not replace the color-inspection presentation or alter the saved provider artifact. The application preserves provider texture pixels and material inputs; it does not blindly brighten, repaint, or discard them to resemble the source photograph.
- An appearance collection or preview failure retains available source and generated artifacts for recovery and legitimate downloads. Retrying collection or reopening the viewer must not submit or charge for another TRELLIS generation; a new paid reconstruction begins only through the existing explicit new-generation action.
- Preserve the source association, source-photo hash, and the relationship between the downloadable `GLB`, portable OBJ package, geometry-only `OBJ`, and manifest. The geometry-only OBJ remains the exact shape handoff and legacy artifact; the validated GLB is the paired appearance source for color-aware LEGO conversion. The interface does not call an unaccompanied OBJ a colored model, claim exact source-photo color recovery, physically accurate materials, or accurate unseen surfaces. Inferred hidden-surface appearance is subject to the same approximation warning as inferred geometry.
- Existing saved jobs remain readable with the artifacts they already contain. A legacy neutral or stripped `GLB` remains viewable and downloadable as-is, with truthful appearance status; the application does not automatically repeat paid TRELLIS work to recreate missing color or textures.
...
MVP non-goals for the standalone photo-to-mesh stage are LEGO brick fitting, parts lists, assembly instructions, new account-management UI, a project-library experience, manual mesh editing, application-authored texture generation or repair, exact color-fidelity claims, arbitrary-object guarantees, provider-specific advanced controls, live pricing, and purchasing. Color-aware LEGO fitting belongs to the connected F8 pipeline. Existing Sites identity plus minimum private job and artifact storage are in scope for secure generation and recovery. Hosted runtime validation, production retention verification, generation timeout, and acceptable generalized quality, latency and cost thresholds remain pending.

Implementation status from current `reconstruction-site` source inspection:

The appearance-preserving behavior above is implemented in the canonical Workshop [collector](site/lib/jobs.ts), [GLB validator](site/core/src/glb.ts) and [viewer](site/app/model-viewer.tsx). [Color preservation QA](docs/qa-trellis-color.md) records texture persistence, owner-checked downloads, geometry parity and browser evidence using labeled fixtures. Those checks do not establish new live inference fidelity. The legacy `reconstruction-site` evidence below remains neutral and does not authorize rerunning or migrating saved jobs.
...
- For a color-aware conversion, identify that brick colors are the closest supported manufactured LEGO colors derived from the reconstructed appearance. Show source-appearance and palette status in text outside the canvas; do not imply pixel-exact photo matching.
...
- Every listed color is the authored LDraw color on the canonical placement and a verified manufactured combination for that part. Show a readable catalog color name beside its swatch; never substitute a display-only RGB color into the shopping list.
...
- Preserve each placement's catalog color through every cumulative step and per-step quantity. A color-aware final step must reproduce the same colored model and part/color totals shown in Model and Parts.
...
- Visiting the canonical workshop without signing in exposes the established navigation, visual style, hand-authored sample and browser-local Admin inspection. Selecting New project opens the real one-photo workflow on a same-origin workshop route rather than the former session-only photo mockup. A user is asked to sign in only before starting paid reconstruction or opening owner-private project data, and returns to the intended context afterward.
- My projects is the minimum recovery surface for jobs created on the canonical workshop. After sign-in, a user can reopen their current/latest job and prior ready job when one exists, with enough stage and status context to avoid repeating paid work. Opening an item resumes its recorded stage and artifact identities; an empty state leads to New project. This slice does not promise a browsable history or full project-library experience.
- A supported image creates one traceable pipeline job. The canonical Workshop does not reject a new attempt because that person has completed five generations in the preceding 24 hours. It still rejects new work after 20 site-wide generation attempts in the rolling window, prevents one user from starting a second unresolved generation, and treats an idempotent replay as the original request rather than another paid attempt. The interface reports truthful stages for image preparation, TRELLIS reconstruction, awaiting brick conversion, conversion, validation and viewer preparation, and identifies which completed stage failed with an actionable retry path.
- The validated appearance-bearing `GLB`, exact geometry-only `OBJ` and manifest associated with the successful TRELLIS result are retained as one downstream handoff. The normalized source image and its checksum, paired mesh hashes, color settings and any later LDraw result remain associated through stable identifiers and integrity metadata; retrying a stage cannot silently mix geometry, appearance or results from different attempts.
- For new canonical Workshop jobs, retain the appearance-bearing TRELLIS `GLB` alongside the geometry-only `OBJ` handoff. The ordinary-model viewer preserves supported original materials and textures, and the owner can download that same colored `GLB` plus a self-contained `OBJ` / `MTL` / texture ZIP. The ZIP reproduces supported material base colors, vertex colors and texture appearance with relative local references; the standalone geometry-only OBJ remains available and is labeled accordingly. When the mesh has no authored appearance, the viewer uses a legible neutral fallback and reports that limitation. Authored appearance that is malformed, unsupported, or cannot be decoded causes a clear collection or preview failure; it is never discarded to turn the result into a neutral success. Available artifacts remain retained for recovery and legitimate downloads, and retrying collection or preview does not repeat paid TRELLIS generation. The interface does not claim exact source color recovery or reliable appearance on inferred hidden surfaces.
- Color-aware conversion samples the validated GLB appearance over the same transformed surface represented by the paired OBJ, including supported material factors, texture coordinates and vertex colors. It deterministically maps visible-surface samples to a versioned palette of named LDraw colors. The result is described as the closest supported LEGO colors, not the exact colors of the photograph or reconstructed texture.
- The confirmed palette direction expands beyond the original eight-color safety set. The initial 26-color target is black, white, light and dark bluish gray; reddish brown, dark brown, tan, dark tan, medium nougat and light nougat; red, dark red, orange, dark orange and yellow; blue, dark blue, medium blue, bright light blue, green and dark green; bright pink, lavender and dark purple; plus dark turquoise and medium azure for teal/cyan subjects. A color enters a published palette only after the fallback and any selected shaped part/color lots have reviewed manufactured evidence. The converter should use only the coherent subset needed by the source rather than maximizing the number of colors in every model.
- Every fitted placement must use a part/color combination with reviewed manufactured-color evidence. If a fitted part is unavailable in the nearest palette color, the fitter may choose a smaller or alternate supported part in that color, or the closest available color under the documented deterministic mapping; it must not invent a part/color lot. If no bounded valid choice exists, conversion fails clearly and retains the reconstructed mesh for retry.
- Color expansion is not limited to basic rectangular bricks. The fitter may select verified bricks, plates, tiles, slopes, curved slopes, wedges, round elements and other supported shape families when their geometry and connection rules improve the result. Part shape and color are selected together: each emitted lot must exist, fit the occupied region, and preserve the applicable collision and supported-placement checks. A narrowly bounded fixed-shape addition may enter this pass only with explicit geometry, manufactured lot evidence and regression coverage; this quick pass does not require a new general solver for arbitrary curved, hinged or articulated families.
- Unobserved, interior and appearance-absent regions use an explicit deterministic fallback color policy recorded in the result. Legacy neutral jobs remain readable and may be reconverted only from appearance they actually retain; the application does not imply that discarded colors can be recovered without a new paid reconstruction.
- Legacy saved jobs continue to open with their existing appearance. Jobs whose stored mesh is neutral or has had appearance stripped remain usable and downloadable without an automatic paid reconstruction. Color preservation begins with newly processed canonical Workshop results and does not imply migration of the legacy pilot Site.
...
- The model workspace renders the bricks and their catalog colors from that `.ldr`, derives its part/color inventory from the same revision and preserves every authored `STEP` / `ROTSTEP` group for direct, previous and next navigation. Each step preserves authored placement colors; the final step reconstructs the full colored model and its part/color totals match Parts exactly.
- Switching among 3D mesh, Brick model, Parts and Instructions keeps the saved LDraw revision loaded. Returning to Instructions preserves the selected draft step for that project, and the project/job/conversion query identity remains unchanged. A result-tab switch must not submit another TRELLIS request, rerun the converter, or require the user to regenerate the model; reload and project recovery continue to use the immutable saved result.
...
- Every new workshop job and derived conversion request is written through the canonical Site's owner-checked D1/R2 boundary. Generation, job, file and conversion operations require a signed-in ChatGPT user and enforce owner access wherever a private record exists. Browser requests cannot choose an owner, object key, callback URL or converter destination. State-changing requests enforce the authenticated owner and accepted workshop Origin. Public sample and Admin-local activity does not create private server records or invoke a paid provider.
...
Integration non-goals are exact photographic color reproduction, printed patterns or decals, arbitrary-object guarantees, manual mesh or brick editing, live pricing or purchasing, public sharing, broad account management, migration of legacy pilot jobs, and removal or redirection of the legacy pilot Site. The external authenticated Python boundary reuses the existing converter on an isolated service in the existing Railway Hobby workspace. Its compute is metered within that plan; no new subscription, image provider or paid inference fallback was added.

Confirmed color extension: the ordinary reconstructed output retains its authored appearance, the owner receives both colored GLB and portable OBJ/MTL/texture downloads, and the converter receives the paired validated GLB plus exact geometry-only OBJ. It quantizes visible appearance to supported manufactured LEGO colors and carries those canonical placement colors through LDraw, Parts and Draft instructions. This is an approximate palette mapping and does not establish shape resemblance, connection legality, stability or physical buildability.

Confirmed quick color/catalog pass: use the founder-supplied object photo and its saved GLB, plus the earlier cat evidence, to check the color-inspection presentation, expanded-palette mapping, useful shaped part selection and result-tab persistence without purchasing another reconstruction. The object benchmark should retain its dominant white body, teal/cyan panels, orange accents and gray handles as distinct broad regions; exact gradients and printed details remain outside the promise. This direction is implemented only when automated checks and a direct review of those supplied artifacts show the accepted behavior above; documentation alone is not implementation evidence. The supplied user assets remain private evaluation inputs rather than repository content.

## UI and interaction principles

The source-color extension is implemented with [independent pipeline QA](docs/qa-end-to-end-colors.md): the portable textured OBJ bundle, paired appearance/geometry conversion, manufactured part/color fitting, and matching colored inventory/draft steps pass their source and fixture checks. Hosted verification and mechanical-build claims remain separate evidence.
