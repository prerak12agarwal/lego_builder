...
Confirmed founder direction: join the existing image reconstruction and LDraw result experience into one user journey on the dedicated integration branch, while Prerak optimizes the Python converter separately. The current slice connects upload, TRELLIS reconstruction, persistent mesh handoff and the model/parts/instructions result surfaces around a stable converter boundary. It must run without a converter by retaining the real geometry-only `OBJ` and truthfully reporting that brick conversion is awaiting the converter. The converter implementation and optimization in PR #5 are explicitly deferred from this slice.
...
- The result boundary can later receive a genuine converter response for the retained mesh without requiring another paid reconstruction. A pipeline result is successful only when that response contains a parseable `.ldr` revision with authored assembly steps and the required validation evidence. A result with missing steps may still be inspected as an incomplete output, using the F7 missing-steps message, but is not presented as a complete build.
...
No automatic Python execution is connected. The owner-only manual LDraw receiver exists to exercise the future adapter and is explicitly labeled as unverified integration tooling; using it does not prove image-to-LEGO conversion or make the supplied file a validated canonical build. Target size is currently a bounded request input from 4–48 studs, not a supported-quality promise, and the source up axis defaults to unknown. The Python execution host remains undecided. A narrow local check reopened a stored bottle mesh and reached its persisted awaiting state without another provider invocation; an isolated temporary LDraw fixture exercised the result UI. Independent [pipeline QA](reconstruction-site/test/pipeline-evidence.md) passes for this deferred-converter slice; hosted and end-to-end conversion acceptance remain separate gates.
